...
Share a folder from the new phone/laptop → set the old phone's folder type to "Receive Only" so it acts as a safe backup target (accidental deletions elsewhere won't wipe the phone's copy).
Syncthing auto-discovers peers over the Tailscale network — no manual IP/port setup required.


4. Serve files over HTTP
